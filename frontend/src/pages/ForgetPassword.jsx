            ...
            <div className='bg-white shadow-md rounded-xl p-8 max-w-md w-full'>
                ...
                <form className='space-y-4' onSubmit={(e)=>e.preventDefault()}>
                    <div>
                        ...
                        focus:ring-[black]' placeholder='xyz123@gmail.com' required
                        onChange={(e) => setEmail(e.target.value)} value={email}></input>
                    </div>
                    <button className='w-full bg-[black] text-white py-2  px-4 font-medium cursor-pointer
                     rounded-md hover:bg-[#4b4b4b] ' disabled = {loading} 
                     onClick={sendOtp}>{loading ? <ClipLoader size={30} color='white'/> : "Send OTP"}</button>
                </form>
            ...
            <div className='bg-white shadow-md rounded-xl p-8 max-w-md w-full'>
                ...
                <form className='space-y-4' onSubmit={(e)=>e.preventDefault()}>
                    <div>
                        ...
                        className='mt-1 w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2
                        focus:ring-[black]' placeholder='* * * *' required
                        onChange={(e) => setOtp(e.target.value)} value={otp}></input>
                    </div>
                    <button className='w-full bg-[black] text-white py-2  px-4 font-medium cursor-pointer
                     rounded-md hover:bg-[#4b4b4b]' disabled = {loading}
                     onClick={verifyOTP}>{loading ? <ClipLoader size={30} color='white'/> : "Verify OTP"}</button>
                </form>
            ...
            <div className='bg-white shadow-md rounded-xl p-8 max-w-md w-full'>
                ...
                <form className='space-y-4' onSubmit={(e)=>e.preventDefault()}>
                    ...
                    <div>
                        ...
                        className='mt-1 w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2
                        focus:ring-[black]' placeholder='*********' required
                        onChange={(e) => setConPassword(e.target.value)} value={conPassword}></input>
                    </div>
                    <button className='w-full bg-[black] text-white py-2  px-4 font-medium cursor-pointer
                     rounded-md hover:bg-[#4b4b4b]' disabled = {loading} 
                     onClick={resetPassword}>{loading ? <ClipLoader size={30} color='white'/> : "Reset Password"}</button>
                </form>
