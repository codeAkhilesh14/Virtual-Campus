...
const Courses = () => {
  ...
  // console.log("Creator Courses:", creatorCourseData);
    ...
    <div className='flex min-h-screen bg-gray-100'>
      <div className='w-[100%] min-h-screen p-4 sm:p-6 bg-gray-100'>
        <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3'>
          ...
          onClick={()=>navigate('/createcourses')}>Create Courses</button>
        </div>
              ...
              {creatorCourseData?.map((course, index)=>(
                <tr key={index} className='border-b hover:bg-gray-50  transition duration-200'>
                  <td className='py-3 px-4 flex items-center gap-4'>
                    {course?.thumbnail ? 
                    <img src={course?.thumbnail} alt="img" className='w-25 h-14 object-cover rounded-md object-fit'/> : 
                    <img src={img} alt="img" className='w-25 h-14 object-cover rounded-md object-fit'/>}
                  ...
                  <td className='px-4 py-3'>
                    ...
                    onClick={()=>navigate(`/editcourse/${course?._id}`)}/> {/* courseid bhi pass karna h */}
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          {creatorCourseData?.map((course, index)=>(
            <div key={index} className='bg-white rounded-lg shadow p-4 flex flex-col gap-3'>
              <div className='flex gap-4 items-center'>
                {course?.thumbnail ?
                <img src={course?.thumbnail} className='w-16 h-16 rounded-md object-cover' alt="img" /> :
              ...
              <span className={`w-fit px-3 py-1 text-xs rounded-full bg-red-100 text-red-600
                ${course?.isPublished ? "bg-green-600 text-green-600" : "bg-red-100 text-red-600"}`}>{course?.isPublished ? "Published" : "Draft"}</span>
            </div>
          ))}
