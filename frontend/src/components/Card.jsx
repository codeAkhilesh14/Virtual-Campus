...
const Card = ({thumbnail, title, category, price, id, reviews}) => {
  ...
  const avrRating = calculateAvgReview(reviews)

  const navigate = useNavigate()


  return (
    <div className="max-w-sm w-full bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-lg transition-all
    ...
    onClick={() => navigate(`/viewcourse/${id}`)}>
      ...
      <div className='p-5 space-y-2'>
        ...
        <div className='flex justify-between text-sm text-gray-600 mt-3 px-[10px]'>
            ...
            <span className='flex items-center gap-1'><FaStar className='text-yellow-500' />{avrRating}</span>
        </div>
      </div>
    </div>
  )
