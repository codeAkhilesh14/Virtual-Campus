    ...
    <div>
      <div className='w-[100%] h-[70px] fixed top-0 px-[20px] py-[10px] flex items-center justify-between bg-[#00000047] z-10'>
        ...
        <div className='w-[40%] lg:w-[20%] lg:pl-[50px]'>
            <img src={logo} alt="Logo" className='w-[60px] rounded-[5px] border-2 border-white' onClick={()=>navigate('/')} />
        </div>
        ...
        <div className='w-[30%] lg:flex items-center justify-center gap-4 hidden'>
            ...
            border-2 bg-black border-white cursor-pointer' onClick={() => setShow(prev=>!prev)} >
                ...
                {/* {(userData?.name || userData?.username || userData?.fullName || "U")[0].toUpperCase()} */}
            </div> }
            ...
            { !userData ?  <span className='px-[20px] py-[10px] border-2 lg:border-white border-black lg:text-white bg-[#000000d5] text-black
            rounded-[10px] text-[18px] font-light cursor-pointer' onClick={() => navigate('/login')}>Login</span> :
            ...
            { show && <div className='absolute top-[110%] right-[15%] flex items-center flex-col justify-center gap-2 text-16px
            rounded-md bg-[white] px-[15px] py-[10px] border-[2px] border-black hover:border-white hover:text-white cursor-pointer
            hover:bg-black'>
        ...
        <div className={`fixed top-0 left-0 w-[100vw] h-[100vh] bg-[#000000d6] flex items-center justify-center flex-col gap-5 z-10 lg:hidden ${showHam ? 'translate-x-[0] transition duration-600' : 'translate-x-[-100%] transition duration-600'}`}>
            ...
            <div className='w-[200px] h-[65px] border-2 border-white text-white bg-[black] flex items-center justify-center
             rounded-[10px] text-[18px] font-light cursor-pointer'
             onClick={() => navigate('/profile')}>My Profile</div>

             <div className='w-[200px] h-[65px] border-2 border-white text-white bg-[black] flex items-center justify-center
             rounded-[10px] text-[18px] font-light cursor-pointer' onClick={()=>navigate('/mycourses')}>My Courses</div>

            { userData?.role === "educator" && <div className='w-[200px] h-[65px] border-2 border-white text-white bg-[black]
             flex items-center justify-center rounded-[10px] text-[18px] font-light cursor-pointer'
             onClick={()=>navigate('/dashboard')}>Dashboard</div> }

             { !userData ?  <span className='w-[200px] h-[65px] border-2 border-white text-white bg-[black]
             flex items-center justify-center rounded-[10px] text-[18px] font-light cursor-pointer' onClick={() => navigate('/login')}>Login</span> :

            <span className='w-[200px] h-[65px] border-2 border-white text-white bg-[black]
             flex items-center justify-center rounded-[10px] text-[18px] font-light cursor-pointer' onClick={handleLogOut}>LogOut</span> }
