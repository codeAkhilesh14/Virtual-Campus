...
function ReviewPage() {
  ...
  useEffect(() => {
    if (reviewData?.length) {
      // Filter reviews that have both user and course data
      const validReviews = reviewData
        .filter(review => review.user && review.course)
        .slice(0, 6);
  ...
  return (
    <div className='flex items-center justify-center flex-col'>
      ...
      <span className='lg:w-[50%] md:w-[80%] text-[15px] text-center mt-[30px] mb-[30px] px-[20px]'>
        Discover how our Virtual Campus is transforming learning experiences through real feedback from students 
